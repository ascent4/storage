-- Variables 
    ...
    local Camera, Client, GuiOffset = Workspace.CurrentCamera, Players.LocalPlayer, GuiService:GetGuiInset().Y
    local Game_Mt, Game_Index, FindFirstChild, FindFirstChildWhichIsA, FindFirstChildOfClass, IsA = getrawmetatable(game), getrawmetatable(game).__index, getrawmetatable(game).__index(game, "FindFirstChild"), getrawmetatable(game).__index(game, "FindFirstChildWhichIsA"), getrawmetatable(game).__index(game, "FindFirstChildOfClass"), getrawmetatable(game).__index(game, "IsA")
    local Mouse = Client:GetMouse()

    -- Data types
    local Vec2, Vec3, Dim2, Dim, Rect, DimOffset = Vector2.new, Vector3.new, UDim2.new, UDim.new, Rect.new, UDim2.fromOffset
  ...
  -- Library init
      ...
      local themes = {
          preset = {
              ...
              accent = Rgb(128, 70, 164); -- 50, 119, 186 for blue
      ...
      local Fonts = {}; do
          function RegisterFont(Name, Weight, Style, Asset)
              local FontsDir = Library.Directory .. "/Fonts/"
              local TtfPath = FontsDir .. Asset.Id
              local FontJsonPath = FontsDir .. Name .. ".font"

              if not isfile(TtfPath) then
                  writefile(TtfPath, Asset.Font)
              end

              if isfile(FontJsonPath) then
                  delfile(FontJsonPath)
              end
              ...
              writefile(FontJsonPath, game:GetService("HttpService"):JSONEncode(Data))

              return getcustomasset(FontJsonPath)
          end
              
          local ProggyTiny = RegisterFont("ProggyTiny", 200, "Normal", {
              Id = "ProggyTiny.ttf",
              Font = game:HttpGet("https://github.com/ascent4/storage/raw/refs/heads/main/fonts/ProggyTiny.ttf"),
          })
  
          local ProggyClean = RegisterFont("ProggyClean", 200, "normal", {
              Id = "ProggyClean.ttf",
          ...
          local Tahoma = RegisterFont("Tahoma", 400, "Normal", {
              Id = "Tahoma.ttf",
              Font = game:HttpGet("https://github.com/ascent4/storage/raw/refs/heads/main/fonts/fs-tahoma-8px.ttf"),
          }) 

          local VerdanaBold = RegisterFont("VerdanaBold", 400, "Normal", {
              Id = "VerdanaBold.ttf",
              Font = game:HttpGet("https://github.com/ascent4/storage/raw/refs/heads/main/fonts/VerdanaBold.ttf"),
          })  

          local Silkscreen = RegisterFont("Silkscreen", 400, "Normal", {
              Id = "Silkscreen.ttf",
              Font = game:HttpGet("https://github.com/ascent4/storage/raw/refs/heads/main/fonts/Silkscreen.ttf"),
          })  
          ...
          Fonts = {
              ...
              ["Verdana Bold"] = Font.new(VerdanaBold, Enum.FontWeight.Regular, Enum.FontStyle.Normal);
              ["Silkscreen"] = Font.new(Silkscreen, Enum.FontWeight.Regular, Enum.FontStyle.Normal);
          }

      end
    Library.Font = Fonts["Tahoma"]
      ...
      -- Library element functions
          ...
          function Library:Dropdown(properties) 
              ...
              function Cfg.SetVisible(bool)
  				...
  				Items.DropdownElements.Size = DimOffset(Items.Outline.AbsoluteSize.X + 1, 0)
      ...
      -- Notification Library
          ...
          function Notifications:RefreshNotifications() 
              ...
              for i, v in Notifications.Notifs do
                  ...
                  Library:Tween(v, {Position = DimOffset(Position.X, Position.Y)})
          ...
          function Notifications:Create(properties)
              ...
              Items.Outline.Position = DimOffset(20, offset)
...
-- Utility
    function Utility:New(Type, Props, Secondarg)
        ...
        local NewFunction = IsDrawing and Drawing.new or Instance.new
    ...
    function Utility:GetCase(Text, CaseType)
    ...
    function Utility:GetHealth(Player, Humanoid)
    ...
    function Utility:GetLocalPlayerCharacter()
        return Utility:GetCharacter(Client)
    end
    ...
    function Utility:GetDistance(Part1, Part2)
        return (Part1.Position - Part2.Position).Magnitude / 3.5714285714
    end

    function Utility:GetBoundingBoxCorners(PartOrModel)
        if not PartOrModel then return nil end
        
        local IsModel = PartOrModel:IsA("Model")
        local BoundingBox, BoxSize
        
        if IsModel then
            BoundingBox, BoxSize = PartOrModel:GetBoundingBox()
        else
            BoundingBox = PartOrModel.CFrame
            BoxSize = PartOrModel.Size
        end
        ...
        for _, Corner in ipairs(Corners) do
            table.insert(WorldCorners, BoundingBox:PointToWorldSpace(Corner))
        end
        
        return WorldCorners, BoxSize
    end

    function Utility:GetScreenBounds(PartOrModel)
        local WorldCorners = Utility:GetBoundingBoxCorners(PartOrModel)
        if not WorldCorners then return nil end
...
-- Visuals
  -- Player ESP  
      ...
      local ESP = function(Player)
          local Box = Utility:New("Frame", { Parent = ScreenGui, Visible = false, BackgroundTransparency = 1, BorderSizePixel = 0 })
          local BoxFill = Utility:New("Frame", { Parent = Box, Visible = true, BackgroundTransparency = 1, Position = UDim2.new(0, -1, 0, -1), Size = UDim2.new(1, 2, 1, 2), BorderSizePixel = 0 })
          local BoxOutline = Utility:New("UIStroke", { Parent = Box, ApplyStrokeMode = Enum.ApplyStrokeMode.Border, LineJoinMode = Enum.LineJoinMode.Miter, Thickness = 3, Color = Color3.fromRGB() })
          local BoxInline = Utility:New("UIStroke", { Parent = BoxFill, ApplyStrokeMode = Enum.ApplyStrokeMode.Border, LineJoinMode = Enum.LineJoinMode.Miter, Thickness = 1, Color = Color3.fromRGB(255, 255, 255) })
          
          local Name = Utility:New("TextLabel", {Parent = ScreenGui, Position = UDim2.new(0.5, 0, 0, -11), Size = UDim2.new(0, 100, 0, 20), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundTransparency = 1, TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = Flags["ESP_Font_Size"], TextStrokeTransparency = 1, TextStrokeColor3 = Color3.fromRGB(0, 0, 0), RichText = true})
          local NameOutline = Utility:New("UIStroke", { Parent = Name, ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual, LineJoinMode = Enum.LineJoinMode.Miter, Thickness = 1, Color = Color3.fromRGB(0, 0, 0) })
  
          local Distance = Utility:New("TextLabel", {Parent = ScreenGui, Position = UDim2.new(0.5, 0, 0, 11), Size = UDim2.new(0, 100, 0, 20), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundTransparency = 1, TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = Flags["ESP_Font_Size"], TextStrokeTransparency = 1, TextStrokeColor3 = Color3.fromRGB(0, 0, 0), RichText = true})
          local DistanceOutline = Utility:New("UIStroke", { Parent = Distance, ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual, LineJoinMode = Enum.LineJoinMode.Miter, Thickness = 1, Color = Color3.fromRGB(0, 0, 0) })
  
          local Weapon = Utility:New("TextLabel", {Parent = ScreenGui, Position = UDim2.new(0.5, 0, 0, 31), Size = UDim2.new(0, 100, 0, 20), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundTransparency = 1, TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = Flags["ESP_Font_Size"], TextStrokeTransparency = 1, TextStrokeColor3 = Color3.fromRGB(0, 0, 0), RichText = true})
          local WeaponOutline = Utility:New("UIStroke", { Parent = Weapon, ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual, LineJoinMode = Enum.LineJoinMode.Miter, Thickness = 1, Color = Color3.fromRGB(0, 0, 0) })
  
          local Flag = Utility:New("TextLabel", {Parent = ScreenGui, Position = UDim2.new(0.5, 0, 0, 31), Size = UDim2.new(0, 100, 0, 20), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundTransparency = 1, TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = Flags["ESP_Font_Size"], TextStrokeTransparency = 1, TextStrokeColor3 = Color3.fromRGB(0, 0, 0), RichText = true})
          local FlagOutline = Utility:New("UIStroke", { Parent = Flag, ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual, LineJoinMode = Enum.LineJoinMode.Miter, Thickness = 1, Color = Color3.fromRGB(0, 0, 0) })
  
          local Healthbar = Utility:New("Frame", {Parent = ScreenGui, BackgroundColor3 = Color3.fromRGB(255, 255, 255), BackgroundTransparency = 0, BorderSizePixel = 0})
          local HealthbarOutline = Utility:New("Frame", {Parent = ScreenGui, ZIndex = -1, BackgroundColor3 = Color3.fromRGB(0, 0, 0), BackgroundTransparency = 0, BorderSizePixel = 0})
          local HealthBarGradient = Utility:New("UIGradient", {Enabled = false, Rotation = 90, Parent = Healthbar})
  
          local Healthtext = Utility:New("TextLabel", {Parent = ScreenGui, Position = UDim2.new(0.5, 0, 0, 31), Size = UDim2.new(0, 100, 0, 20), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundTransparency = 1, TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = Flags["ESP_Font_Size"], TextStrokeTransparency = 1, TextStrokeColor3 = Color3.fromRGB(0, 0, 0), RichText = true})
          local HealthtextOutline = Utility:New("UIStroke", { Parent = Healthtext, ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual, LineJoinMode = Enum.LineJoinMode.Miter, Thickness = 1, Color = Color3.fromRGB(0, 0, 0) })

          local Chams = Utility:New("Highlight", {Parent = ScreenGui, FillTransparency = 1, OutlineTransparency = 0, OutlineColor = Color3.fromRGB(255, 255, 255), DepthMode = "AlwaysOnTop"})
          --
          local EspUpdate = function()
              local Connection;
              ...
              --
              Connection = Utility:Connect(RunService.RenderStepped, function()
                  if Player.Character and Utility:GetRootPart(Player.Character) then
                      ...
                      local Dist = Utility:GetDistance(Camera.CFrame, HumanoidRootPart)
  
                      local Bounds = Utility:GetScreenBounds(Character)
  
                      if OnScreen and Dist <= Flags["ESP_Max_Distance"] then
                          local w, h = Bounds.Width * 0.95, Bounds.Height
  
                          -- Teamcheck
                          if Flags["ESP_Enabled"] and Flags["ESP_Team_Check"] and Player ~= Client and (Utility:GetTeam(Client) ~= Utility:GetTeam(Player) or (Utility:GetTeam(Client) == nil and Utility:GetTeam(Player) == nil)) then 
                              
                              do -- Box
                                  Box.Position = UDim2.new(0, Pos.X - w / 2, 0, Pos.Y - h / 2)
                                  Box.Size = UDim2.new(0, w, 0, h)
                                  Box.Visible = Flags["ESP_Box"]
                                  BoxInline.Color = Flags["ESP_Box_Color"].Color
                              end
      
                              do -- Healthbar
                                  local HealthBarValue = Health / MaxHealth;

                                  Healthbar.Visible = Flags["ESP_Healthbar"];
                                  Healthbar.Position = UDim2.new(0, Pos.X - w / 2 - 7, 0, Pos.Y - h / 2 + h * (1 - HealthBarValue) - 2) 
                                  Healthbar.Size = UDim2.new(0, 1, 0, h * HealthBarValue + 4)
                                  Healthbar.BackgroundColor3 = Flags["ESP_Healthbar_Color"].Color 
                                  if Flags["ESP_Healthbar_Gradient"] then
                                      HealthBarGradient.Enabled = true
                                      HealthBarGradient.Color = ColorSequence.new({
                                           ColorSequenceKeypoint.new(0, Flags["ESP_Healthbar_Gradient_Top_Color"].Color),
                                           ColorSequenceKeypoint.new(1, Flags["ESP_Healthbar_Gradient_Bottom_Color"].Color),
                                      })
                                  ...
                                  HealthbarOutline.Visible = Flags["ESP_Healthbar"];
                                  HealthbarOutline.Position = UDim2.new(0, (Pos.X - w / 2 - 7) - 1, 0, (Pos.Y - h / 2) - 3)  
                                  HealthbarOutline.Size = UDim2.new(0, 1 + 2, 0, h + 6)
  
                                  do -- Healthtext
                                      Healthtext.Visible = Flags["ESP_Healthbar_Healthtext"] and Health < MaxHealth
                                      Healthtext.Text = tostring(math.floor(Health))
                                      Healthtext.Position = UDim2.new(0, Healthbar.Position.X.Offset + Healthbar.Size.X.Offset / 2, 0, Healthbar.Position.Y.Offset - 7)
                                      Healthtext.AnchorPoint = Vector2.new(0.5, 0)
                                      Healthtext.TextColor3 = Flags["ESP_Healthbar_Healthtext_Color"].Color
                                      Healthtext.FontFace = Fonts[Flags["ESP_Font"]]
                                      Healthtext.TextSize = Flags["ESP_Font_Size"]
                                  end
  
                              end
  
                              do -- Name
                                  Name.Visible = Flags["ESP_Name"];
                                  Name.Text = Utility:GetCase(Player.Name, Flags["ESP_Text_Casing"])
                                  Name.Position = UDim2.new(0, Pos.X, 0, Pos.Y - h / 2 - 9)
                                  Name.TextColor3 = Flags["ESP_Name_Color"].Color
                                  Name.FontFace = Fonts[Flags["ESP_Font"]]
                                  Name.TextSize = Flags["ESP_Font_Size"]
                              end
                              
                              do -- Weapon
                                  Weapon.Position = UDim2.new(0, Pos.X, 0, Pos.Y + h / 2 + 6)
                                  Weapon.Text = Utility:GetCase(Utility:GetToolName(Player, Character) or "none", Flags["ESP_Text_Casing"])
                                  Weapon.Visible = Flags["ESP_Weapon"];
                                  Weapon.TextColor3 = Flags["ESP_Weapon_Color"].Color;
                                  Weapon.FontFace = Fonts[Flags["ESP_Font"]]
                                  Weapon.TextSize = Flags["ESP_Font_Size"]
                              end
  
                              do -- Distance
                                  Distance.Position = UDim2.new(0, Pos.X, 0, Pos.Y + h / 2 + (Weapon.Visible and 16 or 7))
                                  Distance.Text = Utility:GetCase(string.format("%dst", math.floor(Dist)), Flags["ESP_Text_Casing"])
                                  Distance.Visible = Flags["ESP_Distance"]
                                  Distance.TextColor3 = Flags["ESP_Distance_Color"].Color
                                  Distance.FontFace =  Fonts[Flags["ESP_Font"]]
                                  Distance.TextSize = Flags["ESP_Font_Size"]
                              end

                              do -- Flags
                                  if Flags["ESP_Flags"] then
                                      local FlagText = Player.Team and Player.Team.Name or "No Team"

                                      local rightX = Pos.X + w / 2 - 24
                                      local topY = Pos.Y - h / 2 - 9

                                      Flag.Visible = true
                                      Flag.Text = Utility:GetCase(FlagText, Flags["ESP_Text_Casing"])
                                      Flag.TextColor3 = Flags["ESP_Flags_Color"].Color
                                      Flag.FontFace = Fonts[Flags["ESP_Font"]]
                                      Flag.TextSize = Flags["ESP_Font_Size"]
                                      Flag.AnchorPoint = Vector2.new(0, 0)
                                      Flag.Position = UDim2.new(0, rightX, 0, topY)
                                  else
                                      Flag.Visible = false
                                  end
                              end
                              ...
                              do -- Chams
                                  ...
                                  Chams.Enabled = Flags["ESP_Chams"]
                                  Chams.FillColor = Flags["ESP_Chams_Color"].Color
                                  Chams.OutlineColor = Flags["ESP_Chams_Outline_Color"].Color
                                  Chams.FillTransparency = 1 - Flags["ESP_Chams_Color"].Transparency -- Use colorpicker transparency
                                  Chams.OutlineTransparency = 1 - Flags["ESP_Chams_Outline_Color"].Transparency -- Use colorpicker transparency
                              end
          ...
          coroutine.wrap(EspUpdate)();
      end
      
      do -- Update ESP
          for _, v in pairs(Utility:GetPlayers()) do
              if v ~= Client then
                  coroutine.wrap(ESP)(v)
              end      
          end
      
          Utility:Connect(Players.PlayerAdded, function(v)
              coroutine.wrap(ESP)(v)
          end, "PlayerAdded");
      end
  --

  -- World, Camera Visuals
    Utility:BindToRenderStep("WorldVisuals", Enum.RenderPriority.Last.Value, function()
      -- Star Count
      if Flags["World_Stars"] then
          local Sky = lighting:FindFirstChildOfClass("Sky")
          if Sky then
              Sky.StarCount = Flags["World_Stars_Amount"]
          end
      end
  
      -- Ambient Lighting
      if Flags["World_Ambient_Enable"] then
        Lighting.Ambient = Flags["World_Ambient_Color"].Color
        Lighting.OutdoorAmbient = Flags["World_Outdoor_Ambient_Color"].Color
      end
        
      -- Brightness
      if Flags["World_Brightness_Enable"] then
        Lighting.Brightness = Flags["World_Brightness_Value"]
      end
        
      -- Fog
      if Flags["World_Fog_Enable"] then
        Lighting.FogStart = Flags["World_Fog_Start"]
        Lighting.FogEnd = Flags["World_Fog_End"]
        Lighting.FogColor = Flags["World_Fog_Color"].Color
      end
        
      -- Time
      if Flags["World_Time_Enable"] then
        Lighting.ClockTime = Flags["World_Time_Value"]
      end
        
      -- Color Correction
      if Flags["World_Color_Correction_Enable"] then
        Lighting.ColorShift_Top = Flags["World_Color_Top"].Color
        Lighting.ColorShift_Bottom = Flags["World_Color_Bottom"].Color
      end
  
      -- Exposure Compensation
      if Flags["World_Exposure_Enable"] then
        Lighting.ExposureCompensation = Flags["World_Exposure_Value"]
      end
  
      -- Fov Changer
      if Flags["Camera_Zoom"] and Flags["Camera_Zoom_Key"] or Flags["Camera_Fov"] then
          DefaultFovAmount = Utility:Lerp(DefaultFovAmount, Flags["Camera_Zoom"] and Flags["Camera_Zoom_Key"] and Flags["Camera_Zoom_Amount"] or Flags["Camera_Fov_Amount"])
          Camera.FieldOfView = DefaultFovAmount
      else
          DefaultFovAmount = Utility:Lerp(DefaultFovAmount, Original.FieldOfView)
          Camera.FieldOfView = DefaultFovAmount
      end
      
      -- Aspect Ratio
      if Flags["Camera_Aspect_Ratio"] then
          local X, Y, Z, R00, R01, R02, R10, R11, R12, R20, R21, R22 = Camera.CFrame:GetComponents()
          Camera.CFrame = CFrame.new(X, Y, Z, 
              R00 * Flags["Camera_Aspect_Ratio_Horizontal"] / 100, 
              R01 * Flags["Camera_Aspect_Ratio_Vertical"] / 100, 
              R02, 
              R10, 
              R11 * Flags["Camera_Aspect_Ratio_Vertical"] / 100, 
              R12, 
              R20 * Flags["Camera_Aspect_Ratio_Horizontal"] / 100, 
              R21 * Flags["Camera_Aspect_Ratio_Vertical"] / 100, 
              R22)
      end 
    end)
  --
...
-- Ui Elements
  -- Window
    local Window = Library:Window({Name = ("Cascade - %s - (%s)"):format(Cascade.Username, game.PlaceId)})
  ...
  -- Combat
    ...
    local Tab1, Tab2, Tab3 = Tabs.Combat:MultiSection({Tabs = {"Hi", "Hello", "Sick"}, Side = "Right"})
  ...
  -- Visuals
    -- ESP Toggles
    local VisualsSection = Tabs.Visuals:Section({Name = "Player ESP", Side = "Left"}) do 
        local ESPToggle = VisualsSection:Toggle({Name = "Enabled", Flag = "ESP_Enabled", Default = true, Folding = true})

        ESPToggle:Toggle({Name = "Name", Flag = "ESP_Name", Default = false})
        :Colorpicker({Name = "Name Color", Flag = "ESP_Name_Color", Color = Rgb(255, 255, 255)})

        ESPToggle:Toggle({Name = "Bounding Box", Flag = "ESP_Box", Default = false})
        :Colorpicker({Name = "Box Color", Flag = "ESP_Box_Color", Color = Rgb(255, 255, 255)})

        ESPToggle:Toggle({Name = "Healthbar", Flag = "ESP_Healthbar", Default = false})
        :Colorpicker({Name = "Healthbar Color", Flag = "ESP_Healthbar_Color", Color = Rgb(0, 255, 0)})

        local HealthBarGradient = ESPToggle:Toggle({Name = "Healthbar Gradient", Flag = "ESP_Healthbar_Gradient", Default = false})
        HealthBarGradient:Colorpicker({Name = "Healthbar Gradient Top Color", Flag = "ESP_Healthbar_Gradient_Top_Color", Color = Rgb(0, 255, 0)})
        HealthBarGradient:Colorpicker({Name = "Healthbar Gradient Bottom Color", Flag = "ESP_Healthbar_Gradient_Bottom_Color", Color = Rgb(255, 0, 0)})

        ESPToggle:Toggle({Name = "Health Number", Flag = "ESP_Healthbar_Healthtext", Default = false})
        :Colorpicker({Name = "Health Number Color", Flag = "ESP_Healthbar_Healthtext_Color", Color = Rgb(255, 255, 255)})
    
        ESPToggle:Toggle({Name = "Tool", Flag = "ESP_Weapon", Default = false})
        :Colorpicker({Name = "Weapon Color", Flag = "ESP_Weapon_Color", Color = Rgb(255, 255, 255)})

        ESPToggle:Toggle({Name = "Distance", Flag = "ESP_Distance", Default = false})
        :Colorpicker({Name = "Distance Color", Flag = "ESP_Distance_Color", Color = Rgb(255, 255, 255)})

        ESPToggle:Toggle({Name = "Flags", Flag = "ESP_Flags", Default = false})
        :Colorpicker({Name = "Flags Color", Flag = "ESP_Flags_Color", Color = Rgb(255, 255, 255)})
        ESPToggle:Dropdown({Name = "Flags", Flag = "ESP_Flags_Type", Default = "Team", Options = {"Team", "Status"}})

        local ChamsToggle = ESPToggle:Toggle({Name = "Chams", Flag = "ESP_Chams", Default = true})
        ChamsToggle:Colorpicker({Name = "Chams Color", Flag = "ESP_Chams_Color", Color = Rgb(125, 125, 125), Alpha = 0.5}) -- Add default alpha
        ChamsToggle:Colorpicker({Name = "Chams Outline Color", Flag = "ESP_Chams_Outline_Color", Color = Rgb(56, 56, 56), Alpha = 0.5}) -- Add default alpha

        ESPToggle:Dropdown({Name = "Chams Type", Flag = "ESP_Chams_Type", Default = "Highlight", Options = {"Highlight", "Adornment"}})

        ESPToggle:Slider({Name = "Render Distance", Flag = "ESP_Max_Distance", Default = 1250, Min = 0, Max = "5000", Increment = 10})
    end
    
    -- ESP Settings
    local ESPSettings, ColorOverrides = Tabs.Visuals:MultiSection({Tabs = {"ESP Settings", "Color Overrides"}, Side = "Left"}) do
        ESPSettings:Toggle({Name = "Team Check", Flag = "ESP_Team_Check", Default = true})

        ESPSettings:Dropdown({Name = "Font", Flag = "ESP_Font", Default = "ProggyTiny", Options = {"ProggyTiny", "ProggyClean", "Verdana", "Tahoma", "Verdana Bold", "Silkscreen"}})
        ESPSettings:Slider({Name = "Font Size", Flag = "ESP_Font_Size", Default = 9, Min = 1, Max = 25, Increment = 1})

        ColorOverrides:Toggle({Name = "Target Color", Flag = "ESP_Target_Color_Enabled", Default = false})
        :Colorpicker({Name = "Target Color", Flag = "ESP_Target_Color", Default = Rgb(255, 0, 0)})

        ESPSettings:Dropdown({Name = "Text Casing", Flag = "ESP_Text_Casing", Default = "Normal", Options = {"Normal", "UPPERCASE", "lowercase"}})
    end

    local World, Camera = Tabs.Visuals:MultiSection({Tabs = {"World", "Camera"}, Side = "Right"}) do
  
      -- World Visuals
        local AmbientToggle = World:Toggle({Name = "Ambient", Flag = "World_Ambient_Enable", Callback = function(state) if not state then Lighting.Ambient = Original.Ambient Lighting.OutdoorAmbient = Original.OutdoorAmbient end end})
        AmbientToggle:Colorpicker({Name = "Ambient Color", Color = Lighting.Ambient, Flag = "World_Ambient_Color"})
        AmbientToggle:Colorpicker({Name = "Outdoor Ambient", Color = Lighting.OutdoorAmbient, Flag = "World_Outdoor_Ambient_Color"})
        
        World:Toggle({Name = "Brightness", Flag = "World_Brightness_Enable", Callback = function(state) if not state then Lighting.Brightness = Original.Brightness end end})
        World:Slider({Name = "Brightness Value", Flag = "World_Brightness_Value", Min = 0, Max = 10, Default = Lighting.Brightness, Decimal = 0.1})
        
        local FogToggle = World:Toggle({Name = "Fog", Flag = "World_Fog_Enable", Callback = function(state) Lighting.FogStart = state and Flags["World_Fog_Start"] or Original.FogStart Lighting.FogEnd = state and Flags["World_Fog_End"] or Original.FogEnd Lighting.FogColor = state and Flags["World_Fog_Color"].Color or Original.FogColor end})
        FogToggle:Colorpicker({Name = "Fog Color", Color = Lighting.FogColor, Flag = "World_Fog_Color"})
        World:Slider({Name = "Fog Start", Flag = "World_Fog_Start", Min = 0, Max = 1000, Default = Lighting.FogStart})
        World:Slider({Name = "Fog End", Flag = "World_Fog_End", Min = 0, Max = 10000, Default = Lighting.FogEnd})
        
        World:Toggle({Name = "Time", Flag = "World_Time_Enable", Callback = function(state) if not state then Lighting.ClockTime = Original.ClockTime end end})
        World:Slider({Name = "Time", Flag = "World_Time_Value", Min = 0, Max = 24, Default = Lighting.ClockTime, Decimal = 0.1})
        
        local ColorCorrectionToggle = World:Toggle({Name = "Color Correction", Flag = "World_Color_Correction_Enable", Callback = function(state) if not state then Lighting.ColorShift_Top = Original.ColorShift_Top Lighting.ColorShift_Bottom = Original.ColorShift_Bottom end end})
        ColorCorrectionToggle:Colorpicker({Name = "Top Color", Color = Lighting.ColorShift_Top, Flag = "World_Color_Top"})
        ColorCorrectionToggle:Colorpicker({Name = "Bottom Color", Color = Lighting.ColorShift_Bottom, Flag = "World_Color_Bottom"})
        
        World:Toggle({Name = "Exposure", Flag = "World_Exposure_Enable", Callback = function(state) if not state then Lighting.ExposureCompensation = Original.ExposureCompensation end end})
        World:Slider({Name = "Exposure Value", Flag = "world_exposure_value", Min = -5, Max = 5, Default = Lighting.ExposureCompensation, Decimal = 1})
        
        if sethiddenproperty and gethiddenproperty then local TechnologyType = tostring(gethiddenproperty(Lighting, "Technology")):gsub("Enum.Technology.", "") World:Dropdown({Name = "Lighting Technology Type", Options = {"Legacy", "Voxel", "Compatibility", "ShadowMap", "Future"}, Default = TechnologyType, Flag = "World_Tech_Type", Callback = function(v) if v and sethiddenproperty then sethiddenproperty(Lighting, "Technology", Enum.Technology[v]) end end}) end
        
        World:Toggle({Name = "Star Count", Flag = "World_Stars_Count"})
        World:Slider({Name = "Star Amount", Flag = "World_Stars_Count_Amount", Min = 100, Max = 5000, Default = 3000, Decimal = 100})
        
      --
      
      -- Camera Visuals
        Camera:Toggle({Name = "Fov Changer", Flag = "Camera_Fov", Callback = function(state) if not state then DefaultFovAmount = Original.FieldOfView Camera.FieldOfView = DefaultFovAmount end end})   
        Camera:Slider({Name = "Fov Amount", Flag = "Camera_Fov_Amount", Min = 1, Max = 120, Default = Camera.FieldOfView})
  
        Camera:Toggle({Name = "Zoom", Flag = "Camera_Fov_Zoom"}):Keybind({Name = "Zoom Key", Callback = function(state) Flags["Camera_Zoom_Key"] = state end, ShowInList = true})
        Camera:Slider({Name = "Zoom Amount", Flag = "Camera_Fov_Zoom_Amount", Min = 1, Max = 120, Default = 30})
  
        Camera:Toggle({Name = "Aspect Ratio", Flag = "Camera_Aspect_Ratio"})
        Camera:Slider({Name = "Horizontal", Flag = "Camera_Aspect_Ratio_Horizontal", Min = 1, Max = 200, Default = 100, Suffix = "%"})
        Camera:Slider({Name = "Vertical", Flag = "Camera_Aspect_Ratio_Vertical", Min = 1, Max = 200, Default = 100, Suffix = "%"})
      --
